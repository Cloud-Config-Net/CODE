<?php
/**
 * NET-CLOUD-CONFIG - URL Shortener & Client Sniffer 
 * File Name: index.php
 */
...
// ==========================================
// Smart Redirect & Client Sniffer Radar
// ==========================================
if (isset($_GET['c'])) {
    ...
    if (isset($db[$id])) {
        ...
        if (time() > $entry['expires'] || ($entry['limit'] > 0 && $entry['downloads'] >= $entry['limit'])) {
            ...
            header("HTTP/1.1 410 Gone"); die("This link has expired or reached its limit.");
        }
        ...
        if ($isBrowser) {
            ...
            die('<!DOCTYPE html><html><body style="background:#0f172a; color:#ef4444; text-align:center; padding-top:20%; font-family:sans-serif;"><h1>🛑 403 Access Denied</h1><p>This config link must be imported inside the HTTP Custom app directly.</p></body></html>');
        }
        ...
        // التوجيه المباشر للرابط الأصلي بدلاً من تحميل ملف
        header("Location: " . $entry['target_url'], true, 302);
        exit;
    }
    header("HTTP/1.1 404 Not Found"); die("Link Not Found.");
}
...
// ==========================================
// URL Shortener Handling Logic
// ==========================================
$generatedLinks = [];
if ($isLogged && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['direct_url'])) {
    $limit = (int)$_POST['limit']; 
    $duration = (int)$_POST['duration'];
    $timeUnit = $_POST['time_unit'] ?? 'hours';
    $targetUrl = filter_var($_POST['direct_url'], FILTER_SANITIZE_URL);
    ...
    if (!empty($targetUrl)) {
        ...
        $generatedLinks[] = ['original_name' => $targetUrl, 'link' => $link];
        file_put_contents($dbFile, json_encode($db));
    }
}
?>
...
<body class="min-h-screen text-slate-200 flex items-center justify-center p-0 sm:p-4 relative">
    ...
    <div class="bg-animations">
        ...
        <div class="floating-element text-5xl text-[#51C0C0]" style="left: 70%; animation-duration: 18s; animation-delay: 2s;"><i class="fa-solid fa-link"></i></div>
    ...
    <div class="glass-panel w-full h-full min-h-screen sm:min-h-0 sm:max-w-[28rem] sm:rounded-[2rem] p-6 sm:p-8 relative flex flex-col justify-center transition-all duration-500 z-10">
        ...
        <div class="text-center mt-12 mb-10 w-full flex flex-col items-center relative z-10">
            <h1 class="text-[32px] sm:text-[36px] font-extrabold tracking-widest text-white drop-shadow-lg">
                SHORT<span class="text-[#51C0C0] neon-text-glow ml-1">URL</span>
            </h1>
        ...
        <div class="bg-[#0a0f1c] border border-[#141c2b] rounded-2xl p-4 sm:p-6 mb-8 max-h-[55vh] sm:max-h-[26rem] overflow-y-auto custom-scroll flex flex-col gap-8 relative z-20">
            ...
            <div class="flex flex-col items-center">
                <span class="text-[#64748b] text-[11px] mb-3 font-mono truncate max-w-full">Target: <?= htmlspecialchars($item['original_name']) ?></span>
                
                <div class="flex items-center w-full gap-3">
                    <div class="flex-1 text-center bg-[#0d131f] p-3 rounded-xl border border-[#1e2738]">
                        <a href="<?= $item['link'] ?>" target="_blank" class="text-[#cbd5e1] text-[12px] sm:text-[13px] hover:text-[#51C0C0] font-mono leading-[1.8] break-all block px-1 transition-colors" id="link-<?= $idx ?>">
                            <?= $item['link'] ?>
                        </a>
                    </div>
        ...
        <div class="flex gap-4 mt-auto">
            ...
            <button onclick="copyAll()" class="flex-[2] bg-[#51C0C0] hover:bg-[#43a3a3] text-[#0a0f1c] font-bold py-4 rounded-xl transition btn-glow text-[12px] uppercase tracking-wider flex items-center justify-center">
                 <i class="fa-solid fa-clone mr-2 text-[15px]"></i> COPY LINK
            </button>
        </div>
        ...
        <?php else: ?>
        <form method="POST" class="space-y-6 mt-auto mb-auto relative z-20">
            
            <div>
                <label class="flex items-center text-[11px] text-[#51C0C0] uppercase tracking-widest font-mono mb-2">
                    <i class="fa-solid fa-link mr-2 text-[12px]"></i> Direct Link Raw Config
                </label>
                <input type="url" name="direct_url" required placeholder="https://..." class="w-full bg-[#0d131f] border border-[#1e2738] rounded-xl px-5 py-4 text-[14px] text-[#8a9bb3] focus:outline-none focus:border-[#51C0C0] focus:shadow-[0_0_10px_rgba(81,192,192,0.1)] transition font-mono placeholder-[#2e3c50]">
            </div>

            <div class="space-y-5 pt-2">
                <div>
                    <label class="flex items-center text-[11px] text-[#51C0C0] uppercase tracking-widest font-mono mb-2">
                        <i class="fa-solid fa-download mr-2 text-[12px]"></i> Limit (default 5 times)
                    </label>
                    <input type="number" name="limit" value="5" class="w-full bg-[#0d131f] border border-[#1e2738] rounded-xl px-5 py-4 text-[15px] text-[#8a9bb3] focus:outline-none focus:border-[#51C0C0] focus:shadow-[0_0_10px_rgba(81,192,192,0.1)] transition font-mono placeholder-[#2e3c50]">
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <input type="number" name="duration" placeholder="Duration" value="24" min="1" required class="w-full bg-[#0d131f] border border-[#1e2738] rounded-xl px-4 py-4 text-[15px] text-center text-white focus:outline-none focus:border-[#51C0C0] transition font-mono placeholder-[#2e3c50]">
            ...
            <button type="submit" class="w-full bg-[#51C0C0] hover:bg-[#43a3a3] text-[#0a0f1c] font-bold py-4 rounded-xl transition btn-glow text-[13px] flex items-center justify-center uppercase tracking-widest mt-4">
                <i class="fa-solid fa-wand-magic-sparkles mr-2 text-[16px]"></i> GENERATE SHORT URL
            </button>
        </form>
        <?php endif; ?>
        <?php endif; ?>
